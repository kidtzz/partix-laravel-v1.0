...
function switchAdminHargaTab(tab) {
    const tabKonf = document.getElementById('tabHargaKonfigurasi');
    const tabDaft = document.getElementById('tabHargaDaftar');
    const contentKonf = document.getElementById('contentHargaKonfigurasi');
    const contentDaft = document.getElementById('contentHargaDaftar');

    if (tab === 'konfigurasi') {
        tabKonf.classList.add('active');
        tabKonf.style.color = 'var(--primary-color)';
        tabKonf.style.borderBottom = '2px solid var(--primary-color)';
        
        tabDaft.classList.remove('active');
        tabDaft.style.color = 'var(--text-muted)';
        tabDaft.style.borderBottom = 'none';

        contentKonf.style.display = 'block';
        contentDaft.style.display = 'none';
    } else {
        tabDaft.classList.add('active');
        tabDaft.style.color = 'var(--primary-color)';
        tabDaft.style.borderBottom = '2px solid var(--primary-color)';
        
        tabKonf.classList.remove('active');
        tabKonf.style.color = 'var(--text-muted)';
        tabKonf.style.borderBottom = 'none';

        contentKonf.style.display = 'none';
        contentDaft.style.display = 'flex';
    }
}

function simpanSemuaPengaturanHarga() {
    ...
    const btn = document.getElementById('btnSimpanPengaturanHarga');
    if (btn) { btn.disabled = true; btn.innerHTML = "<i class='bx bx-loader-alt bx-spin'></i> Menyimpan..."; }
    ...
    BackendAPI.call('updatePengaturanDiskon', [payload]).then(() => {
        ...
        if (typeof loadAdminHarga === 'function') {
            loadAdminHarga();
        } else if (typeof adminBarangData !== 'undefined' && adminBarangData && adminBarangData.length > 0) {
        ...
        if (btn) { btn.disabled = false; btn.innerHTML = "<i class='bx bx-save'></i> Simpan Pengaturan"; }
    });
}
...
function renderAdminHarga(data) {
    ...
    tbody.innerHTML = data.map(b => {
        ...
        return `
            <tr>
                <td>
                    <div title="${b.nama_barang}" style="color: var(--text-main); margin-bottom: 8px; max-width: 220px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${b.nama_barang}</div>
                    <div>
                        <span style="background: rgba(79, 70, 229, 0.08); color: var(--primary-color); padding: 2px 6px; border-radius: 4px; font-weight: 400;">${b.id_barang}</span>
                    </div>
                </td>
                <td><span style="color: var(--text-main);">${formatRupiah(b.harga_modal || 0)}</span></td>
                <td><span style="color: var(--text-main);">${formatRupiah(hrgReg)}</span></td>
                <td><span class="price-pill" style="background: rgba(59, 130, 246, 0.08); color: #2563eb;">${formatRupiah(hrgMem)}</span></td>
                <td><span class="price-pill" style="background: rgba(79, 70, 229, 0.08); color: #4f46e5;">${formatRupiah(hrgLan)}</span></td>
                <td><span class="price-pill" style="background: rgba(139, 92, 246, 0.08); color: #7c3aed;">${formatRupiah(hrgBeng)}</span></td>
                <td><span class="price-pill" style="background: rgba(16, 185, 129, 0.08); color: #059669;">${formatRupiah(hrgTem)}</span></td>
                <td><span class="price-pill" style="background: rgba(236, 72, 153, 0.08); color: #db2777;">${formatRupiah(hrgGro)}</span></td>
                <td>
                    <span class="badge ${b.status_harga === 'Aktif' ? 'badge-success' : 'badge-secondary'}" style="font-weight: 400;">${b.status_harga || 'Nonaktif'}</span>
                </td>
                <td style="text-align: center;">
                    <button type="button" class="btn btn-secondary btn-sm" onclick="editHarga('${b.id_barang}')" style="padding: 5px 12px; font-weight: 400;"><i class='bx bx-edit-alt'></i> Edit</button>
                </td>
            </tr>
            `;
    }).join('');
...
function renderAdminBarang(data) {
    ...
    tbody.innerHTML = data.map(b => {
        ...
        let actionButtons = `<button type="button" class="btn btn-secondary btn-sm" style="font-weight: 400;" onclick="editModalBarang('${b.id_barang}')">Edit</button>`;
        ...
        return `
            <tr class="main-row hoverable-row">
                ...
                <td>
                    <div title="${b.nama_barang}" style="max-width: 220px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${b.nama_barang}</div>
                </td>
                <td>
                    <div>${b.lokasi_rak || '-'}</div>
                </td>
                <td>
                    <span class="badge ${badgeClass}" style="font-weight: 400;">${statusVal}</span>
                </td>
...
function renderAdminSupplier(data) {
    ...
    tbody.innerHTML = data.map(s => {
        ...
        if (s.pics && s.pics.length > 0) {
            picContent = s.pics.map(p => `<div style="margin-bottom: 8px; display: flex; align-items: center;"><i class='bx bx-user' style="color:var(--primary-color); margin-right: 6px;"></i> <span>${p.nama || '-'}</span></div>`).join('');
            hpContent = s.pics.map(p => `<div style="margin-bottom: 8px; display: flex; align-items: center;"><i class='bx bx-phone' style="color:var(--text-muted); margin-right: 6px;"></i> <span>${p.hp || '-'}</span></div>`).join('');
        } else {
        ...
        return `
            <tr>
                <td>${s.id_supplier}</td>
                <td><div>${s.nama_supplier}</div></td>
                <td>${picContent}</td>
                <td><div>${hpContent}</div></td>
                <td><span class="badge ${s.status_supplier === 'Aktif' ? 'badge-success' : 'badge-secondary'}" style="font-weight: 400;">${s.status_supplier || 'Aktif'}</span></td>
                <td>
                    <button type="button" class="btn btn-secondary btn-sm" style="font-weight: 400;" onclick="editModalSupplier('${s.id_supplier}')">Edit</button>
                    ${s.status_supplier === 'Nonaktif' || s.status_supplier === 'Non Aktif' ? `<button type="button" class="btn btn-sm" style="background:var(--danger-color);color:white;margin-left:4px; font-weight: 400;" onclick="hapusSupplierPermanen('${s.id_supplier}')" title="Hapus Permanen"><i class='bx bx-trash'></i></button>` : ''}
                </td>
            </tr>
            `;
    }).join('');
    ...
    tbody.innerHTML = data.map(t => {
        ...
        return `
                <tr class="hoverable-row">
                    <td><div>${t.no_invoice}</div></td>
                    <td>${formattedDate}</td>
                    <td>${t.kasir}</td>
                    <td>${t.tipe_harga}</td>
                    <td><span class="badge ${statusBadge}" style="font-weight: 400;">${t.status}</span></td>
                    <td style="color: var(--success-color);">Rp ${Number(t.total).toLocaleString('id-ID')}</td>
                    <td>
                        <button type="button" class="btn btn-secondary btn-sm" style="font-weight: 400;" onclick="cetakUlangTransaksi('${t.no_invoice}')"><i class='bx bx-receipt'></i> Detail</button>
                    </td>
                </tr>
            `;
    }).join('');
...
function cetakUlangTransaksi(noInvoice) {
    ...
    BackendAPI.call('cetakInvoice', [noInvoice]).then(res => {
        ...
        res.items.forEach(d => {
            html += `<tr>
                    <td style="padding:12px; border-bottom:1px solid #e2e8f0; color: var(--text-main);">${d.nama_barang}</td>
                    <td style="padding:12px; border-bottom:1px solid #e2e8f0; text-align:center; color: var(--text-main);">${d.qty}</td>
                    <td style="padding:12px; border-bottom:1px solid #e2e8f0; text-align:right;">Rp ${Number(d.harga_satuan).toLocaleString('id-ID')}</td>
                    <td style="padding:12px; border-bottom:1px solid #e2e8f0; text-align:right; color: var(--text-main);">Rp ${Number(d.subtotal).toLocaleString('id-ID')}</td>
                </tr>`;
        });
        html += `
                    </tbody>
...
function setupIntersectionObserver(tbodyId, stateObj, loadFunc) {
    const tbody = document.getElementById(tbodyId);
    if (!tbody) return;
    
    if (tbody.observer) {
        tbody.observer.disconnect();
    }
    
    tbody.observer = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) {
            if (!stateObj.loading && stateObj.page < stateObj.lastPage) {
                loadFunc(stateObj.page + 1);
            }
        }
    ...
    if (lastRow) {
        tbody.observer.observe(lastRow);
    }
}

let adminLogState = { page: 1, lastPage: 1, loading: false };

function loadAdminLog(page = 1) {
    if (adminLogState.loading) return;
    if (page > 1 && page > adminLogState.lastPage) return;
    ...
    if (page === 1) {
        ...
        tbody.insertAdjacentHTML('beforeend', `<tr id="adminLogLoading"><td colspan="6" style="text-align:center; color: var(--text-muted);"><small><i>Memuat data tambahan...</i></small></td></tr>`);
    }

    BackendAPI.call('getLogActivityAdmin', [page]).then(response => {
        const records = response.data || [];
        adminLogState.page = response.current_page;
        adminLogState.lastPage = response.last_page;
        
        const loadingRow = document.getElementById('adminLogLoading');
        if (loadingRow) loadingRow.remove();

        if (records.length === 0 && page === 1) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">Belum ada log aktivitas.</td></tr>`;
        } else {
            const html = records.map(log => {
                const date = new Date(log.timestamp);
                const formattedDate = date.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' });
                return `
                        <tr>
                            <td>${formattedDate}</td>
                            <td><div>${log.username}</div></td>
                            <td><span class="badge badge-secondary" style="font-size: 12px; font-weight: 400;">${log.role}</span></td>
                            <td>${log.action}</td>
                            <td>${log.module}</td>
                            <td>${log.details}</td>
                        </tr>
    ...
    if (page === 1) loadSystemLog();
}

let isSystemLogLoaded = false;
let globalSystemLogs = [];
let systemLogState = { page: 1, lastPage: 1, loading: false };

function loadSystemLog(page = 1) {
    if (systemLogState.loading) return;
    if (page > 1 && page > systemLogState.lastPage) return;
    ...
    if (page === 1) {
        ...
        tbody.insertAdjacentHTML('beforeend', `<tr id="systemLogLoading"><td colspan="6" style="text-align:center; color: var(--text-muted);"><small><i>Memuat data tambahan...</i></small></td></tr>`);
    }

    BackendAPI.call('getSystemLogs', [page]).then(response => {
        const records = response.data || [];
        
        const baseIndex = (page === 1) ? 0 : globalSystemLogs.length;
        if (page === 1) globalSystemLogs = records;
        else globalSystemLogs = globalSystemLogs.concat(records);
        
        systemLogState.page = response.current_page;
        systemLogState.lastPage = response.last_page;

        const loadingRow = document.getElementById('systemLogLoading');
        if (loadingRow) loadingRow.remove();

        if (records.length === 0 && page === 1) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">Sistem berjalan baik, tidak ada log tercatat.</td></tr>`;
        } else {
            const html = records.map((log, index) => {
                const globalIndex = baseIndex + index;
                const levelClass = log.level === 'error' ? 'color: var(--danger-color);' :
                    (log.level === 'warning' ? 'color: #f59e0b;' : 'color: var(--primary-color);');
                return `
                        <tr>
                            <td>${log.timestamp}</td>
                            <td style="${levelClass}">${log.level.toUpperCase()}</td>
                            <td>${log.user}</td>
                            <td><span style="word-break: break-all;">${log.url}</span></td>
                            <td style="color: var(--danger-color);">${log.message.substring(0, 100)}${log.message.length > 100 ? '...' : ''}</td>
                            <td>
                                <button type="button" class="btn btn-secondary" style="padding: 4px 10px; font-size: 12px; font-weight: 400;" onclick="viewContextSystemLog(${globalIndex})">
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; gap: 8px;">
                        ...
                        ${u.status === 'Nonaktif' ? `<button type="button" class="btn btn-sm" style="background:var(--danger-color);color:white; font-weight: 400;" onclick="hapusUser('${u.username}')"><i class='bx bx-trash'></i></button>` : ''}
                    </div>
                </td>
            </tr>
        `).join('');
